...
class NotificationService(private val config: SmtpConfig?) {
        ...
        send(
            ...
            html = buildHtml(
                ...
                bodyHtml = """
                    ...
                    <p style="$TEXT">Transloom just finished a translation run for <b>${esc(projectName)}</b>.</p>
                    ...
                    ${ctaButton("Open Review Portal", "https://transloom.dev/dashboard#review", secondary = true)}
                """.trimIndent()
            )
        )
    }
    ...
    suspend fun sendReviewQueueReminder(user: User, pendingCount: Int) {
        ...
        send(
            ...
            html = buildHtml(
                ...
                bodyHtml = """
                    <p style="$TEXT">Hi <b>${user.githubUsername}</b>,</p>
                    <p style="$TEXT">You have <b>$pendingCount string${if (pendingCount != 1) "s" else ""}</b> waiting in your Transloom review portal. Once approved, a pull request is created automatically.</p>
                    ${ctaButton("Review Now", "https://transloom.dev/dashboard#review")}
    ...
    suspend fun sendTrialLimitHit(user: User) {
        ...
        send(
            ...
            html = buildHtml(
                ...
                bodyHtml = """
                    <p style="$TEXT">Hi <b>${user.githubUsername}</b>,</p>
                    <p style="$TEXT">Your Transloom account has used all <b>500 free strings</b> for this month. New pushes to your watched branch will be queued but not translated until you upgrade.</p>
                    <table style="$STAT_TABLE">
                    ...
                    ${ctaButton("Upgrade Now", "https://transloom.dev/dashboard#billing")}
    ...
    suspend fun sendCheckoutAbandoned(user: User, pendingPlan: BillingPlan) {
        ...
        send(
            ...
            html = buildHtml(
                ...
                bodyHtml = """
                    <p style="$TEXT">Hi <b>${user.githubUsername}</b>,</p>
                    <p style="$TEXT">You started setting up a <b>${pendingPlan.displayName}</b> trial on Transloom but didn't finish. Your 7-day free trial is still waiting — no charge until the trial ends.</p>
                    ${ctaButton("Complete Setup", "https://transloom.dev/dashboard#billing")}
        ...
        send(
            ...
            html = buildHtml(
                ...
                bodyHtml = """
                    ...
                    ${ctaButton("Manage Billing", "https://transloom.dev/dashboard#billing", secondary = true)}
                """.trimIndent()
            )
        )
    }
    ...
    suspend fun sendOnboardingReminder(user: User, stuckReason: String) {
        ...
        send(
            to = email,
            subject = "Pick up where you left off — Transloom",
            html = buildHtml(
                title = "You're close!",
                bodyHtml = """
                    <p style="$TEXT">Hi <b>${user.githubUsername}</b>,</p>
                    ...
                    ${ctaButton("Continue Setup", "https://transloom.dev/dashboard")}
            ...
            <tr><td align="center">
              <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:8px;box-shadow:0 1px 3px rgba(0,0,0,.1);overflow:hidden;">
                ...
                <tr><td style="background:#18181b;padding:24px 32px;">
                  <span style="font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.5px;">Transloom</span>
                ...
                <tr><td style="background:#f4f4f5;padding:20px 32px;border-top:1px solid #e4e4e7;">
                  <p style="margin:0;font-size:12px;color:#71717a;line-height:1.6;">
                    You're receiving this because you have a Transloom account.<br>
                    <a href="https://transloom.dev/dashboard#settings" style="color:#71717a;">Manage email preferences</a>
                    &nbsp;·&nbsp;
                    <a href="https://transloom.dev" style="color:#71717a;">transloom.dev</a>
                  </p>
                </td></tr>
