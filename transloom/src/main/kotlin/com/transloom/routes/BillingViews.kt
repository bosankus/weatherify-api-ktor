...
internal fun HTML.checkoutPage(
    ...
    body {
        div("co-wrap") {
            ...
            header("co-header") {
                a(href = "/transloom", classes = "co-brand") {
            ...
            main("co-main") {
                ...
                aside("co-stack co-stack-right") {
                    div("co-summary") {
                        ...
                        p("co-pay-foot") { +"By continuing you agree to Transloom's Terms and acknowledge the Privacy Policy." }
                    }
                }
            }
            footer("co-footer") {
            ...
            div("co-expired-overlay") {
                ...
                div("co-expired-card") {
                    ...
                    a("/transloom#pricing", classes = "co-pay-btn co-expired-btn") { +"Restart checkout" }
                }
            }
        }
        script {
            unsafe {
                ...
                (function(){
                  ...
                  function tick(){if(expired)return;var left=Math.max(0,TIMEOUT-(Date.now()-startedAt));if(timerText)timerText.textContent=pad(Math.floor(left/60000))+':'+pad(Math.floor((left%60000)/1000));if(timerFill)timerFill.style.transform='scaleX('+(left/TIMEOUT)+')';if(left===0){expired=!0;fetch('/transloom/billing/cancel-pending',{method:'POST',credentials:'include'});var el=document.getElementById('co-expired');if(el)el.classList.add('co-expired-overlay-show');return}setTimeout(tick,1000)}tick();
                  var cfg={key:${quote(init.keyId)},subscription_id:${quote(init.subscriptionId)},name:'Transloom',description:${quote("${plan.displayName} plan · 7-day free trial")},image:'https://data.androidplay.in/transloom/favicon.svg',prefill:{name:${quote(userName)},email:${quote(userEmail ?: "")}},theme:{color:'#00E5A0',backdrop_color:'#000000'},handler:function(resp){var params=new URLSearchParams({razorpay_payment_id:resp.razorpay_payment_id||'',razorpay_subscription_id:resp.razorpay_subscription_id||${quote(init.subscriptionId)},razorpay_signature:resp.razorpay_signature||''});window.location.href='/transloom/billing/rp-callback?'+params.toString()},modal:{ondismiss:function(){document.getElementById('co-overlay').classList.remove('co-overlay-show');document.getElementById('co-pay').disabled=!1},escape:!0,backdropclose:!1},notes:{plan:${quote(plan.name)}}};
    ...
    body {
        div("co-wrap co-wrap-success") {
            ...
            div("co-success-card") {
                ...
                div("co-success-steps") {
                    ...
                    div("co-success-step") {
                        span("co-step-num") { +"3" }; div("co-step-body") { strong { +"Push a commit" }; p { +"Transloom opens a PR with every language translated automatically." } }
                    }
                }
        ...
        script {
            unsafe {
                ...
                if(activateBtn&&token){activateBtn.addEventListener('click',function(){activateBtn.disabled=!0;activateBtn.textContent='Activating…';fetch('/transloom/api/billing/activate-now',{method:'POST',headers:{'Authorization':'Bearer '+token}}).finally(function(){window.location.href=dashUrl})})}
                """.trimIndent()
            }
        }
    }
...
private const val CHECKOUT_LOGO_SVG = """<svg viewBox="0 0 32 32" width="28" height="28" aria-hidden="true"><defs><linearGradient id="coLg" x1="0" y1="0" x2="32" y2="32" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#00F5B0"/><stop offset="100%" stop-color="#00A87A"/></linearGradient></defs><rect width="32" height="32" rx="8" fill="url(#coLg)"/><path d="M8.5 10.5 H23.5" stroke="#0a0a0a" stroke-width="2.8" stroke-linecap="round"/><path d="M16 10.5 V23" stroke="#0a0a0a" stroke-width="2.8" stroke-linecap="round"/></svg>"""
...
:root{--bg:#0a0a0a;--surface:#121212;--surface2:#1a1a1a;--border:#282828;--accent:#00E5A0;--accent-soft:rgba(0,229,160,.14);--text:#f0f0f0;--text-dim:#a0a0a0;--text-muted:#707070;--radius:16px;--radius-sm:10px}
...
.co-blob-1{background:radial-gradient(circle at 30% 30%,#00E5A0,transparent 60%);top:-200px;left:-200px;animation:coDrift 20s ease-in-out infinite}
...
.co-avatar-fallback{background:linear-gradient(135deg,#00E5A0,#00A87A);display:flex;align-items:center;justify-content:center;color:#0a0a0a;font-weight:700;font-size:14px}
...
.co-summary{position:relative;background:linear-gradient(180deg,var(--surface),rgba(18,18,18,0));border:1px solid var(--border);border-radius:var(--radius);padding:32px;animation:coFade .7s ease .15s both;box-shadow:0 40px 100px -50px rgba(0,229,160,.3),0 0 0 1px rgba(0,229,160,.1) inset}
.co-summary::before{content:'';position:absolute;inset:-1px;border-radius:inherit;padding:1px;background:linear-gradient(180deg,rgba(0,229,160,.5),transparent 70%);-webkit-mask:linear-gradient(#fff 0 0) content-box,linear-gradient(#fff 0 0);-webkit-mask-composite:xor;mask-composite:exclude;pointer-events:none;opacity:.7}
...
.co-summary-badge{display:inline-flex;align-items:center;font-size:12px;font-weight:700;color:var(--accent);background:var(--accent-soft);border:1px solid rgba(0,229,160,.3);padding:5px 12px;border-radius:999px;text-transform:uppercase;letter-spacing:.5px}
...
.co-success-card{position:relative;z-index:2;max-width:520px;background:linear-gradient(180deg,var(--surface),rgba(18,18,18,0));border:1px solid var(--border);border-radius:var(--radius);padding:56px 48px;text-align:center;box-shadow:0 40px 100px -40px rgba(0,229,160,.35)}
...
.co-step-num{flex-shrink:0;width:32px;height:32px;border-radius:50%;background:var(--accent-soft);border:1px solid rgba(0,229,160,.35);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:var(--accent)}
...
.co-timer-label{position:relative;z-index:3;padding:8px 48px;font-size:12px;color:var(--text-muted);background:rgba(0,229,160,.05);border-bottom:1px solid rgba(0,229,160,.1)}
