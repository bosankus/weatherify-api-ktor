...
fun Route.configureApiRoutes(
    ...
    route("/api") {
        ...
        route("/projects") {
            ...
            //
            //   GET /syncling/api/projects/{id}/export?lang=fr&format=auto
            //
            ...
            for (attempt in 1..maxRetries) {
                try {
                    ...
                    val added = githubService.addCommitToBranch(
                        ...
                        files         = updatedFiles,
                        commitMessage = "chore(i18n): add $stringCount reviewed translation${if (stringCount != 1) "s" else ""} for $langCount language${if (langCount != 1) "s" else ""} (approved via Syncling portal)",
                        token         = githubToken
                    )
        ...
        for (attempt in 1..maxRetries) {
            try {
                ...
                val pr = githubService.createBranchAndPr(
                    ...
                    prTitle       = "Syncling: $stringCount reviewed translation${if (stringCount != 1) "s" else ""} · $langCount language${if (langCount != 1) "s" else ""}",
