...
import com.androidplay.weatherify.domain.Payment
 ...
 */
class PaymentRepositoryImpl(private val databaseModule: WeatherifyDb) : PaymentRepository {
    ...
    override suspend fun savePayment(payment: Payment): Result<Boolean> {
        return try {
            databaseModule.getPaymentsCollection().insertOne(payment)
            logger.info("Payment saved: {}", payment.paymentId)
            Result.success(true)
        } catch (e: Exception) {
            logger.error("Failed to save payment: {}", e.message)
            Result.error("Failed to save payment: ${e.message}", e)
        }
    }
}
