# Syncling CDN — Integration Guide

This guide covers everything a developer needs to consume Syncling's global edge-delivered
...
```
git push  →  Syncling pipeline  →  Cloudflare KV (250+ PoPs)
                                          ↓
...
1. Your team pushes a commit — Syncling detects new or changed strings and translates them.
...
```
GET https://cdn.syncling.dev/{projectId}/{locale}/strings.json
```

| Segment     | Description                                              | Example                        |
|-------------|----------------------------------------------------------|--------------------------------|
| `projectId` | The UUID of your Syncling project                       | `a1b2c3d4-…`                   |
| `locale`    | BCP-47 locale code matching your configured target       | `es`, `fr`, `de`, `ja`, `hi`  |

### Example

```
GET https://cdn.syncling.dev/a1b2c3d4-e5f6-7890-abcd-ef1234567890/es/strings.json
```

> **Finding your Project ID** — open the Syncling dashboard → Projects → click your project →
...
```json
{
  "onboarding_welcome_title": "Bienvenido a Syncling",
...
```kotlin
import com.syncling.sdk.Syncling
...
class MyApp : Application() {
    override fun onCreate() {
        ...
        Syncling.init(
            context  = this,
            apiKey   = "YOUR_API_KEY",      // from Syncling dashboard → Settings → API key
...
val saveBtn = Syncling.string("settings_save_button")

// With a fallback for keys not yet published to CDN
val label   = Syncling.string("new_feature_label", default = "New feature")
```
...
Syncling.addUpdateListener { newVersion ->
...
// Package.swift
dependencies: [
    .package(url: "https://github.com/syncling/syncling-ios-sdk", from: "1.0.0")
]
```
...
let title   = Syncling.string("onboarding_welcome_title")
let saveBtn = Syncling.string("settings_save_button")
let label   = Syncling.string("new_feature_label", default: "New feature")
...
```swift
Syncling.onUpdate { version in
    DispatchQueue.main.async {
        // Invalidate and redraw live views
        NotificationCenter.default.post(name: .synclingBundleUpdated, object: nil)
    }
}
```
...
`Library/Caches/com.syncling/` and is cleared only when the OS evicts cache data under storage
pressure.
...
const BASE_URL = 'https://cdn.syncling.dev';
...
import { loadTranslations, t } from './syncling';

export function useSyncling(locale: string) {
...
private const val CDN = "https://cdn.syncling.dev"
private const val PROJECT_ID = "a1b2c3d4-e5f6-7890-abcd-ef1234567890"

class SynclingClient(private val http: HttpClient) {
...
of all locale strings. Syncling only writes a new KV entry when the content actually changes —
...
- The **CDN Status** widget in your Syncling dashboard (right sidebar).
...
The **CDN Status** card in the right sidebar of your Syncling dashboard shows:
...
default to avoid layout shifts. You can call `Syncling.refresh()` from your app code to force
...
The CDN is served by Cloudflare's global network with no Syncling-level rate limit. Cloudflare
...
a bundle well under 50 KB. If you approach the limit, Syncling logs a warning during the CDN
...
Use the **Review Portal** (`/syncling/review-portal`) to approve strings for a specific locale,
...
```bash
curl -I https://cdn.syncling.dev/{projectId}/{locale}/strings.json
# Expect: HTTP/2 200, ETag: "...", X-Bundle-Version: "..."
```

---

*Last updated: 2026-05-20 · Syncling CDN v1*
